 ...
 */
 class RecordVideoPresenter : Presenter<RecordVideoContract.View<*>>() {
    ...
    private var mTimer: Timer? = null//录制计时器
     ...
     override fun initConfiguration(option: RecordVideoOption) {
        mOption = option
     ...
     */
     override fun pressToStartRecordVideo(holder: SurfaceHolder?, ivFlipCamera: AppCompatImageView,
                                                ivFlashlight: AppCompatImageView,xcimg:AppCompatImageView): Boolean {
        if (mCamera == null) {
            startPreview(holder)
        }
        if (isRecording) {
...
//        ivBack.visibility = View.GONE
     ...
     * 释放按钮停止录制视频
     */
     override fun releaseToStopRecordVideo(isCancel: Boolean) {
        //释放方法会执行两次，进行过滤
     ...
     override fun flipCamera(holder: SurfaceHolder?) {
        holder?.let {
     ...
     */
     override fun stopRecordVideo(): Boolean {
        if (!isRecording) {
            return false
        }
...
//        }
     ...
     */
     ...
     override fun getTimingHint(timing: String, isInTiming: Boolean): String {
     ...
     override fun switchFlashlightState(turnOn: Boolean): Boolean {

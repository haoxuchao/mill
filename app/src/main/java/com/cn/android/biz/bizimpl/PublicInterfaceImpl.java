...
public class PublicInterfaceImpl implements PublicInterfaceBiz {
    ...
    @Override
    public void publicTokenPostRequest(Activity context, Map<String, Object> paramsMap, String url, final OnRequestListener requestListener) {
        Map<String, String> params = new HashMap<>();
        L.e("Https", "Post url = " + url);
        if (null != paramsMap) {
            for (Map.Entry<String, Object> entry : paramsMap.entrySet()) {
        ...
        UserBean userBean=new Gson().fromJson(SPUtils.getString(Constant.UserBean,""), UserBean.class);
        L.e("Https", "Post params = " + new Gson().toJson(params));
        OkHttpUtils.post().url(url)
